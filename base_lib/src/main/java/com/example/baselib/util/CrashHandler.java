...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    /**
     * 关键函数,当程序中有未捕获的异常,系统将自动调用uncaughtException,     *     * @param thread    未捕获异常的线程     * @param throwable throwable为未捕获的异常
     */
    @Override
    public void uncaughtException(Thread thread, Throwable throwable) {        //把异常信心写到sdcard
        dumpExceptionToSDCard(throwable);
        //上传服务器
        uploadExceptionToServer();        //打印堆栈
    ...
    private void dumpExceptionToSDCard(Throwable throwable) {        //判断SD卡不存在或无法使用
    ...
    private void dumpDeviceInfo(PrintWriter printWriter) throws PackageManager.NameNotFoundException {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            printWriter.println("App Version: " + packageInfo.versionName + "_" + packageInfo.getLongVersionCode());
        } else {
